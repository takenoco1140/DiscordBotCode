...
<div class=\"wrap\">
  ...
  <div class=\"top\">
    <div class=\"title\">
      <h1>本日のスクリム情報 <span class=\"badge\">{date_badge}</span></h1>
      <div class=\"sub\">
        🟠 回転式｜🔵 従来式｜（登録しない＝アイコン無し）<br>
        {server_name}
      </div>
    </div>
    <div class=\"legend\">
      <span>更新: <b>{updated_at}</b></span>
      <span>予定数: <b>{count}</b></span>
    </div>
  </div>
...
async def _try_render_png_from_html_panel(html: str, width: int = 980, height: int = 560) -> Optional[bytes]:
    try:
        from playwright.async_api import async_playwright
    except Exception:
        return None
        ...
        async def scrim_today_preview(interaction: discord.Interaction):
            ...
            try:
                ...
                for i, png in enumerate(pages, start=1):
                    ...
                    content = f"🧪【本日のスクリム情報｜プレビュー】{suffix}"
                    if interaction.response.is_done():
